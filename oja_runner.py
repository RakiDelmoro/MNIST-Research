...
from cupy_mlp_models.backprop_combined_oja_utils import model
...
def main():
    ...
    TRANSFORM = lambda x: torch.flatten(transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean=[0.5], std=[0.5])])(x)).type(dtype=torch.float32)
