...
def training(dataloader, forward_pass, backward_pass, model_parameters, update_parameters, learning_rate):
    ...
    for input_image_batch, expected_batch in dataloader:
        ...
        per_batch_loss, _ = cross_entropy_loss(model_prediction, expected_batch)
        update_parameters(model_parameters[0], learning_rate, forward_neuron_activations, backward_activations)
    ...
    return cp.mean(cp.array(batched_losses)).item()
...
def runner(epochs, for_training_array_of_tuple, for_validation_array_of_tuple, shuffle, batch_size, update_parameters, forward_pass, backward_pass, model_parameters, learning_rate):
    for epoch in range(epochs):
        ...
        average_loss_for_all_batch = training(training_dataloader, forward_pass, backward_pass, model_parameters, update_parameters, learning_rate)
