...
def axons_initialization(input_feature, output_feature):
    bound_w = torch.sqrt(6) / torch.sqrt(input_feature) if input_feature > 0 else 0
    weights = torch.random.uniform(-bound_w, bound_w, size=(input_feature, output_feature), dtype=torch.float32)
    # weights = cp.random.randn(input_feature, output_feature) * 0.01
    return weights

def dentrites_initialization(output_feature):
    bound_b = 1 / torch.sqrt(output_feature) if output_feature > 0 else 0
    bias = torch.random.uniform(-bound_b, bound_b, size=(output_feature,), dtype=torch.float32)
...
def forward_pass_activations(neurons, layers_parameters):
    ...
    for each in range(total_activations):
        ...
        neurons = linear(neurons, axons.transpose(), dentrites)
...
def update_layers_parameters(neurons_activations, layers_losses, layers_parameters, learning_rate):
    ...
    for layer_idx in range(total_parameters):
        ...
        oja_parameters_nudge = learning_rate * (cp.dot(previous_activation.transpose(), current_activation)
                                                - cp.dot(cp.dot(current_activation.transpose(), current_activation), axons.transpose()).transpose())
