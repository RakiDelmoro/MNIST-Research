...
def cupy_mlp_neural_network(network_feature_sizes, training_loader, validation_loader, learning_rate, epochs, residual_use):
    ...
    residual_indexes = [(2**n) for n in range(8)]
    ...
    network_axons_and_dentrites = residual_axons_and_dentrites_initialization(network_feature_sizes)
    def training_run():
        if residual_use:
            return residual_training_layers(dataloader=training_loader, layers_parameters=network_axons_and_dentrites, residual_idx=residual_indexes, learning_rate=learning_rate)
        else:
    ...
    def test_run():
        if residual_use:
            return residual_test_layers(dataloader=validation_loader, layers_parameters=network_axons_and_dentrites, residual_idx=residual_indexes)
        else:
